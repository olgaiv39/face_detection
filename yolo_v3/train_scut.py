...
from src.scut_dataset import SCUTDataset
...
def get_args():
    ...
    parser.add_argument("--train_set", type=str, default="train",
                        help="For both Part_A and Part_B, you could choose 3 different datasets: train, trainval and val. Additionally, for SCUT_HEAD_Part_A, you could also pick the dataset name test")
    parser.add_argument("--test_set", type=str, default="val",
                        help="For both Part_A and Part_B , you could choose 3 different datasets: train, trainval and val. Additionally, for SCUT_HEAD_Part_A, you could also pick the dataset name test")
    parser.add_argument("--part", type=str, default="Part_A", help="Part of dataset (Part_A  or Part_B)")
    parser.add_argument("--data_path", type=str, default="./data/SCUT_HEAD", help="the root folder of dataset")
    parser.add_argument("--pre_trained_model_type", type=str, choices=["model", "params"], default="model")
    parser.add_argument("--pre_trained_model_path", type=str, default="./trained_models/whole_model_trained_yolo_scut")
    parser.add_argument("--log_path", type=str, default="./tensorboard/yolo_scut")
...
def train(opt):
    ...
    training_set = SCUTDataset(opt.data_path, opt.part, opt.train_set, opt.image_size)
    training_generator = DataLoader(training_set, **training_params)

    test_set = SCUTDataset(opt.data_path, opt.part, opt.test_set, opt.image_size, is_training=False)
    ...
    log_path = os.path.join(opt.log_path, "{}".format(opt.part))
    ...
    for epoch in range(opt.num_epoches):
        ...
        if epoch % opt.test_interval == 0:
            ...
            if te_loss + opt.es_min_delta < best_loss:
                ...
                torch.save(model.state_dict(), opt.saved_path + os.sep + "only_params_trained_yolo_scut")
                torch.save(model, opt.saved_path + os.sep + "whole_model_trained_yolo_scut")
