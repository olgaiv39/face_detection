...
def get_args():
    ...
    parser.add_argument("--part", type=str, default="Part_A", help="The part of dataset (Part_A or Part_B)")
...
def test(opt):
    input_list_path = os.path.join(opt.data_path, "SCUT_HEAD_{}".format(opt.part), "ImageSets/Main/{}.txt".format(opt.test_set))
    image_ids = [id.strip() for id in open(input_list_path)]
    output_folder = os.path.join(opt.output, "SCUT_HEAD_{}_{}".format(opt.part, opt.test_set))
    ...
    for id in image_ids:
        image_path = os.path.join(opt.data_path, "SCUT_HEAD_{}".format(opt.part), "JPEGImages", "{}.jpg".format(id))
